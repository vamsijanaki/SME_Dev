...
class InstructorPayout extends Command
{
    ...
    {
      /*  $oldMonth = Carbon::now(Settings('active_time_zone') )->subMonths(1) ;
        $monthlyData =  $monthlyData =  CourseEnrolled::leftJoin('courses','courses.id','course_enrolleds.course_id')
            ->whereMonth('course_enrolleds.created_at',  Carbon::now(Settings('active_time_zone') )->subMonths(1))->groupBy('courses.user_id')
