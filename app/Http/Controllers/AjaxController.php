    ...
    public function statusEnableDisable(Request $request)
    {
        ...
        try {
            ...
            if ($table == "courses") {
                ...
                $course = Course::find($id);
                if ($status == 1) {
                       if (UserEmailNotificationSetup('Course_Publish_Successfully',Auth::user())) {
                             send_email(Auth::user(), 'Course_Publish_Successfully', [
                                'time' => Carbon::now()->format('d-M-Y ,s:i A'),
                                'course' => $course->title,
                            ]);
                        }
                        if (UserBrowserNotificationSetup('Course_Publish_Successfully',Auth::user())) {

                            send_browser_notification(Auth::user(), $type = 'Course_Publish_Successfully', $shortcodes = [
                                'time' => Carbon::now()->format('d-M-Y ,s:i A'),
                                'course' => $course->title,
                            ],
                            '',//actionText
                        ...
                        if (UserEmailNotificationSetup('Course_Unpublished',Auth::user())) {
                             send_email(Auth::user(), 'Course_Unpublished', [
                                'time' => Carbon::now()->format('d-M-Y ,s:i A'),
                                'course' => $course->title,
                            ]);
                        }
                        if (UserBrowserNotificationSetup('Course_Unpublished',Auth::user())) {

                            send_browser_notification(Auth::user(), $type = 'Course_Unpublished', $shortcodes = [
                                'time' => Carbon::now()->format('d-M-Y ,s:i A'),
                                'course' => $course->title,
                            ],
                            '',//actionText
