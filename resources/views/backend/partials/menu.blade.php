                            ...
                            <select name="code" id="language_code" class="nice_Select bgLess mb-0 menuLangChanger"
                                ...
                                @foreach ($LanguageList as $key => $language)
                                    <option value="{{route('changeLanguage',$language->code)}}"
                                            @if (\Illuminate\Support\Facades\Auth::user()->language_code == $language->code) selected @endif>{{ $language->name }}</option>
                            ...
                            <div class="Menu_NOtification_Wrap notifications_wrap">
                                ...
                                <div class="Notification_body">
                                    ...
                                    @forelse (Auth::user()->unreadNotifications as $notification)
                                        <div class="single_notify d-flex align-items-center" id="menu_notification_show_{{$notification->id}}">
                                            <div class="notify_thumb">
                                                 <i class="fa fa-bell"></i>
                                            </div>
                                            <a href="#" class="unread_notification"  title="Mark As Read" data-notification_id="{{$notification->id}}">
                                                <div class="notify_content">
                                                        <h5>{{ \Illuminate\Support\Str::limit(@$notification->data['title'], 30, $end='...') }}</h5>
                                                    <p>{{ \Illuminate\Support\Str::limit(@$notification->data['body'], 70, $end='...') }}</p>
                                                </div>
                                            </a>
                                        </div>
                                    @empty
                                ...
                                <div class="nofity_footer">
                                    <div class="submit_button text-center pt_20">
                                        <a href="{{route('MyNotification')}}" class="primary-btn radius_30px text_white  fix-gr-bg">See More</a>
                                        <a href="{{route('NotificationMakeAllRead')}}" id="mark_all_as_read" class="primary-btn radius_30px text_white  fix-gr-bg">Mark As Read</a>
                                    </div>
                                </div>
                            </div>
                            <!--/ Menu_NOtification_Wrap  -->
