...
//                $img->resize(270, 181);
    ...
    public function CourseLessonShow($id, $chapter_id, $lesson_id)
    {
        try {
            ...
            if (isModuleActive('Org')) {
                $all['branches'] = OrgBranch::orderBy('order')->get();
            }

            return view('coursesetting::course_details',$all, compact('data', 'editLesson', 'levels', 'video_list', 'vdocipher_list', 'course', 'chapters', 'categories', 'instructors', 'languages', 'course_exercises', 'quizzes', 'certificates'));
    ...
    public function courseDetails($id, $data = null)
    {
        ...
        $data = [];
        if (isModuleActive('Org')) {
            $data['branches'] = OrgBranch::orderBy('order')->get();
        }
        return view('coursesetting::course_details', $data, compact('data', 'vdocipher_list', 'levels', 'video_list', 'course', 'chapters', 'categories', 'instructors', 'languages', 'course_exercises', 'quizzes', 'certificates'));
    ...
    public function getAllCourseData(Request $request)
    {
        ...
        return Datatables::of($query)
            ...
            ->editColumn('scope', function ($query) {
                ...
                if (permissionCheck('course.courseInvitation')  && $query->type == 1) {
            ...
            })->rawColumns(['status', 'price', 'action','enrolled_users'])->make(true);
    }
